...
class StreamsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create stream" do
    assert_difference('Stream.count') do
      post streams_url, params: { stream: { classroom_id: @stream.classroom_id, name: @stream.name } }
    end
  ...
  test "should update stream" do
    patch stream_url(@stream), params: { stream: { classroom_id: @stream.classroom_id, name: @stream.name } }
